...
class ForecastPresenter(
            ...
            disposable.add(forecastWeatherModel.getFiveDayForecast(location.first, location.second)
                ...
                ?.subscribeWith(object : DisposableSingleObserver<FiveDayForecastResponse>() {
                    ...
                    override fun onSuccess(response: FiveDayForecastResponse?) {
                        doOnSuccessForecastResponse(response)
                    }

                    override fun onError(e: Throwable?) {
                        doOnErrorForecastResponse(e)
                    }
    ...
    private fun doOnSuccessForecastResponse(response: FiveDayForecastResponse?) {
        if (response?.forecastList != null) {
            val forecastSectionList = forecastWeatherModel.getForecastSectionList(response.forecastList)
    ...
    private fun doOnErrorForecastResponse(e: Throwable?) {
